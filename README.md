...
A proof of concept daisy-chaining Nextflow workflows: taxprofiler and metaval.

**This is heavily inspired from https://github.com/mahesh-panchal/nf-cascade**

A [guided example is on the original wiki](https://github.com/mahesh-panchal/nf-cascade/wiki/Guided-Example-%E2%80%90-Nf%E2%80%90core-style-workflow)
...
Run nf-core/taxprofiler -> genomic_medicine_sweden/metaval:
```bash
...
- Difficult to start from alignment steps, the pipeline has to complete before starting the new one, but concurrent running is possible.
...
combining workflows in the past, such as [Mahesh Panchal's](https://github.com/mahesh-panchal/test_nfcore_workflow_chain)
a few years ago. At the time, his attempt was not feasible as there was too much maintenance
...
solution here. Mahesh Panchal proposed that one could run `nextflow` in a native process
...
from the launch directory, rather than the work directory. After a bit of googling, Mahesh
discovered the `ProcessBuilder` class, which could run a command in another directory. Initially
Mahesh thought about running this in the work directory (`task.workDir`), but realized any
failures would start the whole child nextflow workflow from the beginning again in a new work directory.
However, since we can write to any directory, making a separate folder in
the work directory in which the workflows could resume, writing only the results the task
specific directory (which is also handy for clean up) would work. Native processes also don't stage
